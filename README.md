...
Docit is a HydePHP-based static site generator for building documentation sites. It builds Markdown and Blade content into a static site and copies the output to your project's `docs/` directory.
...
Or from the project root when docit is at `packages/docit`:
...
Docit includes a reusable workflow for building and deploying docs. Choose one of the following:

### Option A: Reusable workflow (docit in monorepo)

When docit lives at `packages/docit` in your repo, add this to `.github/workflows/docs.yml`:
...
jobs:
  build:
    uses: ./packages/docit/.github/workflows/build-docs.yml
    with:
      docit-path: packages/docit
      commit: true
```

### Option B: Copy workflow to root (Composer-installed docit)

When docit is installed via Composer at `vendor/christhompsontldr/docit`, copy the workflow logic into your root `.github/workflows/docs.yml`:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: '20'
      - name: Build docs
        run: php vendor/christhompsontldr/docit/artisan docit
      - name: Add .nojekyll
